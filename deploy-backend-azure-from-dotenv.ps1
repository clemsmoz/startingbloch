...
<#
  ...
  - Lit un fichier .env (clé=valeur, commentaires '#' supportés)
  - Exécute : build, zip, ressources, app settings, déploiement, test

  Usage :
    powershell -ExecutionPolicy Bypass -File .\deploy-backend-azure-from-dotenv.ps1 `
...
function Read-EnvFile([string]$Path) {
  ...
  Get-Content -LiteralPath $Path | ForEach-Object {
    ...
    # Split au 1er '=' uniquement
    $idx = $line.IndexOf("=")
    if ($idx -lt 1) { return }
    $key = $line.Substring(0, $idx).Trim()
    $val = $line.Substring($idx + 1).Trim()
    # Retire guillemets bord
...
# Construit la liste des settings à appliquer (ignore valeurs vides)
$pairs = @(
...
Write-Host ""
Write-Host "- / :"
try { (curl -sS "$appUrl").Content.Substring(0, [Math]::Min(500, (curl -sS "$appUrl").Content.Length)) } catch {}
